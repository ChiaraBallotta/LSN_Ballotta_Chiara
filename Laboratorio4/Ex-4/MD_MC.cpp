...
int main()
{ 
  string Fase = "gas";   // indica su quale stato stiamo svolgendo la simulazione = SOLID, LIQUID,GAS
...
// distance i-j in pbc
  ...
  walker[iv] = 4.0 * v; // Potential energy, stima relativa alla particella
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin;  // Total energy;
  walker[iw] = rho*walker[it] + (1.0/ (3* vol))*48.0*w;	//pressione
...
// cout << "Acceptance rate " << accepted/attempted << endl << endl;
    ...
    Epot.open("output/"+Fase+ "/output_epot.dat",ios::app);
    Ekin.open("output/"+Fase+"/output_ekin.dat",ios::app);
    Temp.open("output/"+Fase+"/output_temp.dat",ios::app);
    Etot.open("output/"+Fase+"/output_etot.dat",ios::app);
    Press.open("output/"+Fase+"/output_press.dat",ios::app);
    ...
    stima_pres = blk_av[iw]/blk_norm; //Pressione
