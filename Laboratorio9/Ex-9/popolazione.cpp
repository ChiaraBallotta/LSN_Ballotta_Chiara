...
Individuo::Individuo(vector<int> &ind_new){
	for(int i = 0; i < ind_new.size();i++){
		m_ind[i] = ind_new[i];
	}					
}
...
//ordina in ordine decrescente gli individuoi, in base alla length. Quelli con length piu piccola sono posti all' inizio dell'array
